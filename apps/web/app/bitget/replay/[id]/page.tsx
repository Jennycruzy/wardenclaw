...
import { getReplay, getBitgetMandate } from "@/lib/data";
...
export default function ReplayPage({ params }: { params: { id: string } }) {
  const mandate = getBitgetMandate(params.id);
  ...
  return (
    <Shell
      ...
      actions={
        <Link
          href={`/bitget/mandates/${params.id}`}
    ...
    >
      {!replay ? (
        <EmptyState
          title="No audit events found for this mandate"
          hint="The mandate exists but its hash-chained audit events were not found on disk. Re-run the paper agent to regenerate the trail."
        />
      ) : (
        <>
          <Card className="mb-3">
            ...
            <div className="mt-3 grid gap-1.5">
              ...
              {replay.proof.truthAnchors.length === 0 ? (
                ...
                replay.proof.truthAnchors.map((a) => (
                  <code key={a} className="break-all font-mono text-xs text-ink-muted">
                    {a}
                  </code>
                ))
              )}
              {replay.proof.paperOnly ? (
                <Badge tone="warn" className="mt-1 w-fit">
                  Paper-only — no external exchange receipt
                </Badge>
              ) : null}
            </div>
          </Card>
          ...
          {replay.rejections.length > 0 ? (
            <Card className="mb-3">
              <SectionTitle title="Reject codes encountered" />
              <div className="flex flex-wrap gap-2">
                {replay.rejections.flatMap((r, i) =>
                  Object.values(r.output)
                    .filter((v): v is string => typeof v === "string" && v.startsWith("REJECT_"))
                    .map((code) => (
                      <Badge key={`${i}-${code}`} tone="neg" className="font-mono">
                        {code}
                      </Badge>
                    )),
                )}
